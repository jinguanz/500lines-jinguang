...
class TempliteTest(TestCase):
    ...
    def test_elif(self):
        self.try_render(
            "Hi, {% if ned %}NED{% elif mike %}MIKE{% endif %}!",
            {'ned': 1, 'mike': 1},
            'Hi, NED!'
        )
        self.try_render(
        ...
        self.try_render(
            "Hi, {% if ned %}NED{% elif mike %}MIKE{% else %}TONY{% endif %}!",
            {'ned': 0, 'mike': 0},
            'Hi, TONY!'
        )
    ...
    def test_loop_if(self):
        ...
        self.try_render(
            "@{% for n in nums %}{% if n %}Z{% else %}Y{% endif %}{{n}}{% endfor %}!",
            {'nums': [0, 1, 2]},
            "@Y0Z1Z2!"
        )
        self.try_render(
            "X{%if nums%}@{% for n in nums %}{{n}}{% endfor %}{% else %}EMPTY{%endif%}!",
            {'nums': []},
            "XEMPTY!"
        )
    ...
    def test_bas_else(self):
        with self.assertSynErr("Mismatched else statement: '{% else %}'"):
            self.try_render("{% else %}X")
        with self.assertSynErr("Don't understand else: '{% else y %}'"):
            self.try_render("{% if x %}X{% else y %}Y{% endif %}")

    def test_bad_elif(self):
        with self.assertSynErr("Mismatched elif statement: '{% elif %}'"):
            self.try_render("{% elif %}X{% endif %}")
